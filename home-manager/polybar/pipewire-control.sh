...
function getCurSink() {
    getCurrSinkName
    curSink=$(pactl list sinks short | grep $curSinkName | awk '{print $2}')
}
...
function getCurVol() {
    ...
    [ -z "$VOL_LEVEL"] && VOL_LEVEL=$(pactl list sinks | grep -A 15 "$1" | grep 'Volume:' | grep -E -v 'Base Volume:' | awk -F : '{print $3; exit}' | grep -o -P '.{0,3}%' | sed 's/.$//' | tr -d ' ')
}
...
function getNickname() {
    #SINK_NICKNAME=${SINK_NICKNAMES[$curSinkName]:-$(pactl list sinks | grep -A 10 'Sink #'"$1"'' | grep 'Description:' | awk -F : '{print $2; exit}')}
    unset SINK_NICKNAME
    eval SINK_NICKNAME="\$SINK_NICKNAMES_$(echo $(pactl info | awk -F : '/Default Sink:/{print $2}' | sed 's/[[:punct:]]/_/g'))"
    [ -z "$SINK_NICKNAME" ] && SINK_NICKNAME=$(pactl list sinks | grep -A 10 'Sink #'"$1"'' | grep 'Description:' | awk -F : '{print $2; exit}')
}
...
function getIsMuted() {
    ...
    [ -z "$isMuted" ] && isMuted=$(pactl list sinks | grep -A 15 "$1" | awk '/Mute/ {print $2; exit}')
}
...
while [[ "$1" = --* ]]; do
    ...
    case "$arg" in
        ...
        --sink-nicknames-from)
            ...
            ;;
        --sink-nickname)
            eval "SINK_NICKNAMES_$(echo ${val//:*/} | sed 's/[[:punct:]]/_/g')='${val//*:}'"
            ;;
        --format)
	    FORMAT="$val"
            ;;
